...
\begin{enumerate}
    \item \textbf{Chazelle (1991)}: $O(n)$ deterministic, but extremely complex.
    \item \textbf{Seidel (1991)}: $O(n \log^* n)$ randomized, simple implementation.
    \item \textbf{Monotone Decomposition}: $O(n \log n)$ via plane sweep, then $O(n)$ triangulation.
