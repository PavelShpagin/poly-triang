...
\midrule
\multicolumn{5}{c}{\textit{Fallback for unstructured polygons (not O(n) but practical)}} \\
\midrule

\textbf{Simple (General)} & Non-self-intersecting boundary & Monotone decomposition + triangulation & 1978 & Garey et al. \\

\textbf{Simple (General)} & Non-self-intersecting boundary & Seidel randomized trapezoidalization & 1991 & Seidel \\
...
\begin{tabular}{llll}
\toprule
\textbf{Year} & \textbf{Result} & \textbf{Authors} & \textbf{Practical?} \\
\midrule
1975 & Kernel computation for star-shaped & Shamos & Yes \\
1978 & $O(n)$ for monotone polygons & Garey et al. & Yes \\
1978 & $O(n \log n)$ decomposition into monotone & Garey et al. & Yes \\
1981 & $O(n)$ for edge-visible polygons & ElGindy \& Avis & Yes \\
1982 & $O(n)$ for orthogonal polygons & Chazelle & Yes \\
1982 & Spiral (funnel) triangulation & Chazelle & Yes \\
1984 & Ear clipping formalized & Meisters & Yes \\
1986 & $O(n \log \log n)$ triangulation & Tarjan \& Van Wyk & Moderate \\
1988 & $O(n \log^* n)$ randomized & Clarkson et al. & Moderate \\
1991 & $O(n \log^* n)$ randomized (simplified) & Seidel & Yes \\
1991 & $O(n)$ deterministic (general simple) & Chazelle & \textbf{No} \\
\bottomrule
\end{tabular}
\end{center}
...
The question ``Can all simple polygons be triangulated in $O(n)$ time using a \textbf{simple, implementable} algorithm?'' remains open.

\textbf{Theoretical status}: Chazelle (1991) proved $O(n)$ is achievable, but the algorithm is impractical (53 pages, never implemented, ``galactic'').

\textbf{Practical status}: 
\begin{itemize}
    \item Structured classes (convex, monotone, star-shaped, etc.): $O(n)$ with simple algorithms.
    \item Unstructured polygons: $O(n \log n)$ via monotone decomposition, or $O(n \log^* n)$ via Seidel.
\end{itemize}

\textbf{Our contribution}: The structured classes cover the vast majority of practical inputs, so the gap between $O(n)$ and $O(n \log n)$ is rarely encountered.
