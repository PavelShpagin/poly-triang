...
class LinearTriangulator:
    ...
    def _bucket_sort_vertices(self):
        ...
        rng = max_y - min_y
        if rng < 1e-9: return list(range(self.n))
        bucket_count = self.n
    ...
    def classify(self):
        ...
        for i in range(self.n):
            ...
            if bp and bn: v.type = VertexType.SPLIT if reflex else VertexType.START
            elif not bp and not bn: v.type = VertexType.MERGE if reflex else VertexType.END
            else: v.type = VertexType.REGULAR_LEFT if not bp else VertexType.REGULAR_RIGHT
...
def benchmark():
    print("=" * 80)
    print("LINEAR TRIANGULATION (BUCKET SORT + SPLAY) - ROTATION CHECK")
    print("=" * 80)
    print(f"{'Type':<15} {'n':>6} {'Rotations':>10} {'Rot/N':>8} {'Comps':>10} {'Cmp/N':>8} {'Total(s)':>10}")
    
    n = 10000
    pts_base = create_comb_jitter(n)
    
    # Original
    tri = LinearTriangulator(pts_base)
    _, stats = tri.triangulate()
    print(f"{'Jitter 0deg':<15} {n:>6} {stats['splay_ops']:>10} {stats['splay_ops']/n:>8.2f} {stats['comparisons']:>10} {stats['comparisons']/n:>8.2f} {stats['sort_time']+stats['sweep_time']:>10.4f}")
    
    # Rotated 90
    pts_90 = rotate_points(pts_base, 90)
    tri = LinearTriangulator(pts_90)
    _, stats = tri.triangulate()
    print(f"{'Jitter 90deg':<15} {n:>6} {stats['splay_ops']:>10} {stats['splay_ops']/n:>8.2f} {stats['comparisons']:>10} {stats['comparisons']/n:>8.2f} {stats['sort_time']+stats['sweep_time']:>10.4f}")
    
    # Rotated 45
    pts_45 = rotate_points(pts_base, 45)
    tri = LinearTriangulator(pts_45)
    _, stats = tri.triangulate()
    print(f"{'Jitter 45deg':<15} {n:>6} {stats['splay_ops']:>10} {stats['splay_ops']/n:>8.2f} {stats['comparisons']:>10} {stats['comparisons']/n:>8.2f} {stats['sort_time']+stats['sweep_time']:>10.4f}")
