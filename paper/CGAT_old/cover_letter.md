...
Please consider our manuscript, “Practical polygon triangulation in \(O(n + k\log k)\) time,” for publication in *Computational Geometry: Theory and Applications*.
...
- **Reproducible artifact**: the submission includes a self-contained artifact folder (`paper/CGAT/`) with scripts to build binaries, run benchmarks, regenerate tables, and rebuild the PDF, along with a correctness sanity check (non-crossing diagonals and \(n-2\) triangles).
...
All code, benchmark scripts, and generated tables are provided in the repository. The `paper/CGAT/reproduce.sh` script rebuilds the artifact end-to-end.
