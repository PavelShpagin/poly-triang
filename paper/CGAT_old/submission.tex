...
\title{Practical polygon triangulation in \texorpdfstring{$O(n + k\log k)$}{O(n + k log k)} time}
...
Experiments against implementations of Garey et al., Hertel--Mehlhorn, and Seidel show that our method is competitive across polygon families: it is fastest in our convex, dent, and random benchmarks for all reported sizes. We also include the standard linear-time fan triangulation fast path for convex polygons.
\end{abstract}
...
To reduce scheduling noise, we pin each run to a single CPU core (Linux \texttt{sched\_setaffinity} when available); this is applied uniformly to all algorithms.
