...
For convex inputs, our implementation benefits from the linear-time fan fast path and achieves a \textbf{8--10$\times$ speedup} over Garey et al. For star polygons, we are \textbf{28\% faster} at $n=10{,}000$. For random polygons, performance is competitive---within 5\% of Garey et al.\ at $n=10{,}000$, and \textbf{faster} at intermediate sizes. Our method significantly outperforms Hertel--Mehlhorn (which times out beyond $n=5{,}000$ on star polygons), Seidel (which shows high constant factors), and (for large $n$) the quadratic Earcut ear-clipping baseline.
