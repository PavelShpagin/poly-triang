...
\title{Practical polygonal triangulation in $O(n + k\log k)$ Time}
...
The algorithm reformulates monotone decomposition to maintain active monotone chains rather than individual edges: the sweep processes only extremal events, while regular vertices are handled via lazy pointer advancement in $O(n)$ amortized time.
Experiments show a \textbf{8--10$\times$ speedup on convex polygons} and \textbf{about 20\% faster on star polygons} over the standard Garey et al.\ monotone-decomposition baseline. On large random polygons ($n=10{,}000$), we are \textbf{about 25\% faster} than Garey et al.\ and \textbf{about 5$\times$ faster} than Earcut. The key practical advantage is that many real-world polygons are nearly convex or have structured reflex vertices, where our algorithm excels.
\end{abstract}
...
We present an algorithm running in $O(n + k\log k)$ time, where $k$ is the number of local extrema (local maxima, equivalently local minima) with respect to the sweep direction. This interpolates between $O(n)$ for convex polygons ($k = 1$) and $O(n\log n)$ for worst-case polygons ($k = \Theta(n)$), improving upon classical methods when $k = o(n/\log n)$.
...
\label{rem:k-param}
Our running time and analysis are stated in terms of $k$, the number of local extrema (equivalently, local maxima). This parameter directly controls the number of sweep events in the chain-based formulation.
\end{remark}
...
We implemented the algorithm in C++ and compared against: Garey et al.~\cite{garey1978} (monotone decomposition via the \texttt{polypartition} library), Hertel--Mehlhorn~\cite{hertel1983} (convex partition), Seidel~\cite{seidel1991} (incremental randomized triangulation), and the widely used Earcut ear-clipping implementation. All experiments were run with GCC at \texttt{-O3}. To prevent pathological slowdowns from dominating wall-clock time, each baseline run was capped with a fixed timeout; missing entries in tables are shown as ``--''.

Our main random-polygon results are included verbatim from the benchmark harness:

\input{generated/benchmark_table}

For convex inputs, our implementation benefits from the linear-time fan fast path and achieves a \textbf{8--10$\times$ speedup} over Garey et al. For star polygons, we are \textbf{about 20\% faster} at $n=10{,}000$. For random polygons, we are \textbf{about 25\% faster} than Garey et al.\ at $n=10{,}000$, and remain faster across the tested range. Our method significantly outperforms Hertel--Mehlhorn (which times out at large $n$ on star polygons) and (for large $n$) the quadratic Earcut ear-clipping baseline; Seidel exhibits high constant factors in our experiments and is omitted when it exceeds the benchmark time cap.
...
The full results across all polygon families appear below (generated directly from the benchmark harness):
