...
Please consider our manuscript, “A Simple, Practical, and Fast Polygon Triangulation in \(O(n + k\log k)\) Time,” for publication in *Computational Geometry: Theory and Applications (CGTA)*.
...
- **Reproducibility**: the submission includes a complete reproduction package with scripts to build binaries, run benchmarks, regenerate tables, and rebuild the PDF, along with correctness sanity checks (non-crossing diagonals and \(n-2\) triangles).
...
All code, benchmark scripts, and generated tables are provided in the repository. The `reproduce.sh` script rebuilds the package end-to-end.
