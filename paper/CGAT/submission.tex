...
\title{A simpler practical polygon triangulation in \texorpdfstring{$O(n + k\log k)$}{O(n + k log k)} time}
...
The key idea is a chain-based reformulation of monotone decomposition: the sweep maintains active monotone chains rather than individual edges and advances chain pointers lazily, so the work over all regular vertices is $O(n)$ amortized while the sweep performs $O(k\log k)$ balanced-tree work over the $2k$ extrema.
Experiments against implementations of Garey et al.\ (monotone decomposition), Seidel, and a CGAL Hertel--Mehlhorn convex-partition baseline show that our method is competitive across polygon families, with large speedups on nearly convex inputs ($k=O(1)$). We also include the standard linear-time fan triangulation fast path for convex polygons.
\end{abstract}

\begin{keyword}
simple polygon \sep triangulation \sep monotone decomposition \sep instance-sensitive algorithm \sep plane sweep
\end{keyword}
...
The triangulation problem has a rich history spanning four decades. The earliest approaches include the ear-clipping method of ElGindy et al.~\cite{elgindy1985}, which runs in $O(n^2)$ time. Garey et al.~\cite{garey1978} achieved $O(n \log n)$ time through monotone decomposition, which remains the standard practical approach; see de Berg et al.~\cite{deberg2008} for a textbook treatment.

Instance-sensitive bounds are also classical. Hertel and Mehlhorn~\cite{hertel1983} give a plane-sweep triangulation algorithm running in \(O(n + s\log s)\), where \(s\) is the number of \emph{start vertices} (vertices whose two neighbors have larger \(x\)-coordinate). For a simple polygon, \(s\) is bounded by the number of reflex vertices plus one, and their framework also yields a constant-factor approximation for convex partitioning. Our parameter \(k\) is another extrema-count with respect to a sweep direction, and \Cref{sec:complexity} shows \(k \le r+1\) as well.

Subsequent work focused on improving worst-case bounds in \(n\). Tarjan and Van Wyk~\cite{tarjan1988} achieved $O(n \log \log n)$ using sophisticated data structures, later simplified by Kirkpatrick et al.~\cite{kirkpatrick1992}. Randomized approaches proved fruitful: Clarkson et al.~\cite{clarkson1989} gave an $O(n \log^* n)$ expected-time Las Vegas algorithm, which Seidel~\cite{seidel1991} simplified significantly using trapezoidal decomposition. Amato et al.~\cite{amato2001} provided further simplifications of randomized linear-time triangulation.
...
Beyond $n$-based worst-case analysis, instance/output complexity measures have also been considered in triangulation.
Chazelle and Incerpi~\cite{chazelle1984} study triangulation and shape complexity, and Toussaint~\cite{toussaint1990,toussaint1991} gives triangulation algorithms whose running time depends on an \emph{output} measure of the produced triangulation.
Separately, the number of monotone chains/extrema depends on the sweep direction; Shin and Kim~\cite{shin2004} study choosing a direction that optimizes a monotone-chain decomposition.
...
Correctness is therefore best stated as an equivalence to the classical sweep, rather than via ad-hoc visibility regions.

\paragraph{Classical baseline (edge-based sweep)}
...
Each active edge $e$ stores a vertex $\mathit{helper}(e)$, defined as the most recently processed vertex above the sweep line that can ``see'' $e$ from the interior.
As in the classical algorithm~\cite{deberg2008}, diagonals are inserted whenever the relevant helper is a \emph{merge} vertex (this can occur at split, merge, end, and certain regular vertices).
...
\begin{lemma}[Advance simulates regular-vertex processing]
...
If \textsc{Advance} emits a diagonal at a regular vertex (as in \Cref{sec:decomposition}), that diagonal coincides with the diagonal inserted by the classical sweep at that regular vertex (to a pending merge helper).
\end{lemma}
...
The only diagonals emitted inside \textsc{Advance} are of the form $(u, C.\mathit{pending})$ at a regular vertex $u$ with $\mathsf{InteriorRight}(u)$.
By \Cref{def:pending-helper}, $C.\mathit{pending}$ represents the classical merge-helper state of the corresponding active edge on this chain.
The classical sweep inserts exactly this diagonal at $u$ in the regular-vertex case when that helper is a merge vertex~\cite{deberg2008}.
\end{proof}

\begin{definition}[Pending as ``merge-helper'']
\label{def:pending-helper}
In the classical sweep, the only helper values that trigger a diagonal later are merge vertices.
...
\begin{proof}
We compare \Cref{alg:decompose} to the classical sweep run on the same polygon, with the same decreasing-$y$ event order.
...
By definition of our BST keys, the in-order ordering of chains in $T$ is the left-to-right ordering of these intersection points, hence agrees with the classical ordering of the corresponding active edges.
Therefore, when \Cref{alg:decompose} takes ``the predecessor of $v$ in $T$'', it identifies the same geometric object as ``the active edge immediately left of $v$'' in the classical sweep.

\medskip
\noindent\textbf{Step 2: Helper agreement on regular vertices via \textsc{Advance}.}
The classical sweep updates $\mathit{helper}(e)$ at regular vertices when the current active edge on a chain changes to the next edge below, and it may also add a diagonal at a regular vertex when the relevant helper is a merge vertex.
...
By \Cref{lem:advance-sim}, the vertex $\mathit{ih}(C,y)$ equals the last vertex encountered when walking down the chain to height $y$, which is exactly the vertex that the classical sweep would have last assigned as helper on the current edge of that chain unless that helper is a merge vertex awaiting a future diagonal.
We encode precisely this exception via \Cref{def:pending-helper}.
When $\mathit{helper}(e)$ is a merge vertex and the classical sweep would add a diagonal at a regular vertex, our \textsc{Advance} emits the same diagonal using $C.\mathit{pending}$ and clears it (see \Cref{sec:decomposition}).

\medskip
\noindent\textbf{Step 3: Case analysis at extrema events.}
Let $v$ be the next event vertex processed.
\begin{itemize}
    \item \textbf{Start.} In the classical sweep, a start vertex inserts its outgoing edge(s) into the status structure and sets their helpers to $v$. In our representation, the unique left-boundary chain originating at $v$ is inserted into $T$ and its pointer is initialized so that $\mathit{ih}(C,y)=v$ for heights just below $v$; also $C.\mathit{pending}$ is null. Thus the (merge-)helper information agrees.

    \item \textbf{End.} In the classical sweep, the edge terminating at an end vertex is removed, and if its helper is a merge vertex then a diagonal is added from $v$ to that helper. In our representation, we remove the terminating left-boundary chain $R$; if $R.\mathit{pending}\neq \textsc{null}$ we add the same diagonal before removal. Hence the diagonal action agrees.

    \item \textbf{Split.} In the classical sweep, at a split vertex $v$ one finds the active edge $e$ immediately left of $v$ and adds a diagonal from $v$ to $\mathit{helper}(e)$; then $\mathit{helper}(e)$ is set to $v$, and the new edge leaving $v$ is inserted with helper $v$.
    In our representation, $L$ is the predecessor chain (Step 1), so its current edge equals $e(L,v.y)$; by Step 2, the helper used by the classical sweep on that edge is either $L.\mathit{pending}$ (if it is a merge) or $\mathit{ih}(L,v.y)=L.\mathit{curr}.\mathit{upper}$ (otherwise).
    ...
    \item \textbf{Merge.} In the classical sweep, at a merge vertex $v$ one may add a diagonal to the helper of the edge terminating at $v$ if that helper is a merge, removes that edge, then finds the edge immediately left of $v$ and may add a diagonal to its merge helper, finally setting the helper of the left edge to $v$.
...
To compare practical performance between instance-sensitive sweeps, it is useful to separate \emph{(i)} the number of balanced-tree operations from \emph{(ii)} the cost of an order comparison in the tree.
...
We implemented the algorithm in C++ and compared against: Garey et al.~\cite{garey1978} (monotone decomposition via the \texttt{polypartition} library), Hertel--Mehlhorn~\cite{hertel1983} (approximate convex partition via \texttt{CGAL} \texttt{approx\_convex\_partition\_2}, then fan triangulation of each convex piece), and Seidel~\cite{seidel1991} (incremental randomized triangulation). All experiments were run with GCC at \texttt{-O3}. To prevent pathological slowdowns from dominating wall-clock time, each run was capped with a \textbf{10\,s timeout} (missing entries are shown as ``--'').
...
We report \emph{mean $\pm$ standard deviation} of the \emph{elapsed time} (ms) printed by each executable for each $(\text{family}, n)$ configuration (seeds $0,\dots,4$), with \texttt{polygons\_per\_config=5}.
...
The \textsc{Convex} family uses seeded affine images of a regular $n$-gon, and \textsc{Dent} pulls one seeded vertex inward (seeded location and depth).
...
For every benchmark run, our implementation either (i) outputs exactly $n-2$ triangles or (ii) fails explicitly (no hidden fallbacks).
In addition to this necessary sanity check, we include a diagonal-validity checker in the accompanying reproducibility package (\texttt{check\_diagonal\_validity.py}) which samples generated polygons and verifies that the decomposition diagonals do not properly intersect the boundary or each other.
The \texttt{reproduce.sh} script runs this check before benchmarking.

Our main convex, dent, and random results are included verbatim from the benchmark harness:

\input{generated/benchmark_table}

For convex inputs, our implementation includes the standard linear-time fan triangulation fast path, achieving roughly $20\times$ speedup over Garey et al.\ for $n=10{,}000$.
...
Seidel exhibits high constant factors in our experiments; CGAL's Hertel--Mehlhorn convex partition baseline is substantially slower than the monotone-decomposition baselines in our setup.
...
Hertel--Mehlhorn~\cite{hertel1983} & $O(n \log n)$ & via convex partition \\
Seidel~\cite{seidel1991} & $O(n \log^* n)$ exp. & randomized \\
Chazelle~\cite{chazelle1991} & $O(n)$ & impractical \\
\textbf{This paper} & $O(n + k \log k)$ & instance-sensitive (in \#extrema $k$) \\
\bottomrule
\end{tabular}
\end{table}
...
The algorithm extends naturally to polygons with holes: each hole boundary contributes its own set of chains and extrema, and the sorted extrema lists are merged. The analysis carries through with $k$ now denoting the total number of local maxima across all boundaries (equivalently, local minima). See also Tereschenko and Tereschenko~\cite{tereschenko2017} for triangulating regions between polygonal boundaries.

\paragraph{Implementation}
For $k$ close to $n$, a simpler edge-based sweep may have better constants. Improving constants in the high-event regime without sacrificing the $k$-sensitive structure is a key practical direction.
