...
Experiments show a \textbf{strong speedup on convex polygons} over the standard Garey et al.\ monotone-decomposition baseline (tens of $\times$ in our suite). On random and star polygons, our method is typically fastest for $n \ge 500$ in our suite, with Garey et al.\ remaining slightly faster only at very small $n$ due to constant-factor overheads. Across all families, we substantially outperform Hertel--Mehlhorn on larger instances.
\end{abstract}
...
e = t + 1.
\]
Consequently, the number of local maxima is
\[
k = s + t = t + m + 1,
\]
and hence \(k \le r + 1\), since split and merge vertices are reflex and \(t+m \le r\). Moreover, no two local maxima can be adjacent along \(\partial P\), so \(k \le \lfloor n/2 \rfloor\).
\end{lemma}
...
For convex inputs, our implementation benefits from the linear-time fan fast path and achieves a \textbf{large speedup} over Garey et al.\ in our suite (often tens of $\times$). On dent polygons (very small $k$), we are consistently faster from $n=500$ upward in our suite, with results at $n=100$ being essentially tied. On random polygons, our method is fastest for all $n \ge 500$ in our suite; at $n=100$, Garey et al.\ can be marginally faster due to constant overhead. On star polygons, we are fastest for $n \ge 500$ in our suite, while the $n=100$ case is again essentially tied. Seidel exhibits high constant factors in our experiments.
