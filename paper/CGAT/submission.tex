...
Experiments show a \textbf{6--9$\times$ speedup on convex polygons} over the standard Garey et al.\ monotone-decomposition baseline. On large random polygons ($n=10{,}000$), we achieve a \textbf{about 1.2$\times$ speedup} over Garey et al.; on star polygons, performance is competitive and becomes faster at larger $n$ in our suite (up to about \textbf{1.6$\times$} at $n=10{,}000$).
\end{abstract}
...
For convex inputs, our implementation benefits from the linear-time fan fast path and achieves a \textbf{6--9$\times$ speedup} over Garey et al.\ in our suite. For star polygons, performance is competitive and becomes faster at larger $n$ (about \textbf{1.6$\times$} at $n=10{,}000$ in our suite). For random polygons, we achieve a \textbf{about 1.2$\times$ speedup} over Garey et al.\ at $n=10{,}000$ and are typically faster across the tested range. Our method significantly outperforms Hertel--Mehlhorn (which times out at large $n$ on star polygons); Seidel exhibits high constant factors in our experiments.
