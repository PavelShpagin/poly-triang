...
Experiments against implementations of Garey et al., Hertel--Mehlhorn, and Seidel show that our method is competitive across polygon families, with large speedups on nearly convex inputs ($k=O(1)$). We also include the standard linear-time fan triangulation fast path for convex polygons.
\end{abstract}
...
Our contribution is a deterministic sweep-based algorithm with running time $O(n + k \log k)$ parameterized by the number of local extrema $k$, achieved by maintaining chains in the sweep status structure and handling regular vertices lazily.
To the best of our knowledge, this explicit extrema-parameterization of monotone decomposition (together with an implementation-oriented equivalence proof to the textbook sweep) has not been previously stated for polygon triangulation.
...
\EndProcedure
\end{algorithmic}
Here $\mathsf{InteriorRight}(u)$ is the standard regular-vertex side test from the textbook sweep (i.e., the subtype of regular vertices handled like an end vertex); it can be decided from the local boundary configuration at $u$.
...
As in the textbook algorithm~\cite{deberg2008}, diagonals are inserted whenever the relevant helper is a \emph{merge} vertex (this can occur at split, merge, end, and certain regular vertices).
...
If \textsc{Advance} emits a diagonal at a regular vertex (as in \Cref{sec:decomposition}), that diagonal coincides with the diagonal inserted by the textbook sweep at that regular vertex (to a pending merge helper).
\end{lemma}
...
By \Cref{def:pending-helper}, $C.\mathit{pending}$ represents the textbook merge-helper state of the corresponding active edge on this chain.
The textbook sweep inserts exactly this diagonal at $u$ in the regular-vertex case when that helper is a merge vertex~\cite{deberg2008}.
\end{proof}
...
Let $D_{\mathrm{text}}$ be the set of diagonals produced by the textbook edge-based sweep on the same polygon and event order.
...
The textbook sweep updates $\mathit{helper}(e)$ at regular vertices when the current active edge on a chain changes to the next edge below, and it may also add a diagonal at a regular vertex when the relevant helper is a merge vertex.
...
When $\mathit{helper}(e)$ is a merge vertex and the textbook sweep would add a diagonal at a regular vertex, our \textsc{Advance} emits the same diagonal using $C.\mathit{pending}$ and clears it (see \Cref{sec:decomposition}).
