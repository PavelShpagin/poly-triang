...
\begin{abstract}
We present a practical algorithm for triangulating a simple polygon with $n$ vertices in $O(n + k\log k)$ time, where $k$ is the number of local maxima (equivalently, local minima) with respect to the sweep direction.
...
We present an algorithm running in $O(n + k\log k)$ time, where $k$ is the number of local maxima (equivalently, local minima) with respect to the sweep direction. This interpolates between $O(n)$ for convex polygons ($k = 1$) and $O(n\log n)$ for worst-case polygons ($k = \Theta(n)$), and can improve upon classical methods when $k$ is substantially smaller than $n$.
...
The key observation is that the sweep needs to process only local extrema, and regular vertices can be handled implicitly by advancing chain pointers lazily. This yields $2k$ sweep events and $O(n)$ total pointer advancement, with balanced-tree operations totaling $O(k\log k)$.
...
Several works relate running time to geometric properties of the input. Hertel and Mehlhorn~\cite{hertel1983} gave a sweep whose cost depends on the number of start vertices $s$ and runs in $O(n + s\log s)$; for simple polygons, $s \le r+1$. Chazelle and Incerpi~\cite{chazelle1984} study triangulation under shape-complexity measures. Toussaint~\cite{toussaint1990,toussaint1991} gives output-sensitive triangulation algorithms. Our parameter $k$ counts local maxima in the sweep direction; \Cref{sec:complexity} shows \(k \le r+1\) and it can be strictly smaller than \(r\) when many reflex vertices are regular.
...
e = t + 1.
\]
Consequently, the number of local maxima is \(s + t = t + m + 1\), which equals the number of local minima \(e + m\). Thus, setting \(k\) to either count gives \(k \le r + 1\) since split and merge vertices are reflex and \(t+m \le r\).
\end{lemma}
