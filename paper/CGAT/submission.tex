...
\begin{abstract}
We present a practical algorithm for triangulating a simple polygon with $n$ vertices in $O(n + k\log k)$ time, where $k$ is the number of local maxima with respect to the sweep direction.
...
Most practical triangulation pipelines first decompose a polygon into $y$-monotone pieces and then triangulate each monotone piece in linear time; see de~Berg et al.~\cite{deberg2008}. The classical plane sweep of Garey et al.~\cite{garey1978} implements this approach in $O(n\log n)$ time and remains a common baseline. Triangulation of monotone polygons, the linear-time subroutine underlying this pipeline, goes back at least to Fournier and Montuno~\cite{fournier1984}. For smaller inputs, the quadratic ear-clipping method~\cite{elgindy1985} is still attractive for its simplicity.

Several works relate running time to geometric properties of the input. Hertel and Mehlhorn~\cite{hertel1983} gave a sweep whose cost depends on the number of start vertices $s$ and runs in $O(n + s\log s)$; for simple polygons, $s \le r+1$. Chazelle and Incerpi~\cite{chazelle1984} study triangulation under shape-complexity measures, and Toussaint~\cite{toussaint1990,toussaint1991} gives output-sensitive triangulation algorithms. Our parameter $k$ counts local maxima in the sweep direction; \Cref{sec:complexity} shows \(k \le r+1\) and it can be strictly smaller than \(r\) when many reflex vertices are regular.

Worst-case bounds in $n$ have also been improved. Tarjan and Van~Wyk~\cite{tarjan1988} achieved $O(n\log\log n)$ using sophisticated data structures, later simplified by Kirkpatrick et al.~\cite{kirkpatrick1992}. Randomized incremental approaches proved fruitful: Clarkson et al.~\cite{clarkson1989} gave an $O(n\log^* n)$ expected-time Las Vegas algorithm, which Seidel~\cite{seidel1991} simplified significantly via trapezoidal decomposition; Amato et al.~\cite{amato2001} provided further simplifications. The breakthrough came with Chazelle's~\cite{chazelle1991} deterministic $O(n)$ algorithm, but its complexity has limited practical adoption. Keil~\cite{keil2000} surveys polygon decomposition more broadly.

Since both monotone decomposition and the maxima count $k$ depend on the sweep direction, the choice of direction matters; Shin and Kim~\cite{shin2004} study selecting directions that optimize monotone-chain decompositions.
...
The table also reports $k$, the number of local maxima in the sweep direction, computed per instance and aggregated in the same way.
...
\textbf{This paper} & $O(n + k \log k)$ & maxima-sensitive in $k$ \\
\bottomrule
\end{tabular}
\end{table}
