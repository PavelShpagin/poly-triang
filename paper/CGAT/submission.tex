...
Experiments show a \textbf{strong speedup on convex polygons} over the standard Garey et al.\ monotone-decomposition baseline (tens of $\times$ in our suite). On high-event inputs (random and star polygons), the classical edge-based baseline remains faster in our current implementation, while our method stays competitive and substantially outperforms Hertel--Mehlhorn on large instances.
\end{abstract}
...
For convex inputs, our implementation benefits from the linear-time fan fast path and achieves a \textbf{large speedup} over Garey et al.\ in our suite (often tens of $\times$). On dent polygons (very small $k$), we are competitive and often comparable to the monotone-decomposition baseline. On random polygons, our method is competitive and often the fastest for medium and large $n$ in our suite, while Garey et al.\ can remain slightly faster for the smallest $n$. On star polygons, Garey et al.\ is still faster at larger $n$ in our current implementation; nevertheless, our method remains within a small constant factor while substantially outperforming Hertel--Mehlhorn (which times out at large $n$ on star polygons). Seidel exhibits high constant factors in our experiments.
