...
\title{Practical output-sensitive polygon triangulation in \texorpdfstring{$O(n + k\log k)$}{O(n + k log k)} time}
...
The key idea is a chain-based reformulation of monotone decomposition: the sweep maintains active monotone chains rather than individual edges, processes only the $2k$ extrema events, and handles regular vertices implicitly via lazy pointer advancement in $O(n)$ amortized time.
Experiments against Garey et al., Hertel--Mehlhorn, and Seidel show that our implementation dominates the baselines on convex, dent, and random families for all reported sizes, achieving up to $22\times$ speedup over Garey et al.\ on convex polygons and $8\%$--$50\%$ lower running time on random polygons; on star polygons, Garey et al.\ is faster only at $n=500$ and we are fastest for larger sizes.
\end{abstract}
