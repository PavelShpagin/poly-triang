...
Experiments show a \textbf{6--13$\times$ speedup on convex polygons} over the standard Garey et al.\ monotone-decomposition baseline. On large random polygons ($n=10{,}000$), we achieve a \textbf{about 1.4$\times$ speedup} over Garey et al.; on star polygons, we observe a \textbf{1.2--1.7$\times$ speedup} across the tested range.
\end{abstract}
...
\Cref{sec:prelim} defines the event-complexity parameter $k$ and discusses its relationship to reflex vertices. \Cref{sec:algorithm} presents the algorithm. \Cref{sec:correctness} establishes correctness. \Cref{sec:complexity} analyzes complexity. \Cref{sec:experiments} provides experimental evaluation. \Cref{sec:discussion} discusses extensions.
...
For convex inputs, our implementation benefits from the linear-time fan fast path and achieves a \textbf{6--13$\times$ speedup} over Garey et al.\ in our suite. For star polygons, we observe a \textbf{1.2--1.7$\times$ speedup} across the tested range (about \textbf{1.4$\times$} at $n=10{,}000$). For random polygons, we achieve a \textbf{about 1.4$\times$ speedup} over Garey et al.\ at $n=10{,}000$ and are typically faster across the tested range. On dent polygons (very small $k$), we are competitive at small $n$ and become faster as $n$ grows. Our method significantly outperforms Hertel--Mehlhorn (which times out at large $n$ on star polygons); Seidel exhibits high constant factors in our experiments.
...
For $k$ close to $n$, a simpler edge-based sweep may have better constants. A hybrid approach using chains when $k < n/\log n$ balances asymptotic and practical performance.
