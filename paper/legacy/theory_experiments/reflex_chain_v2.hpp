#pragma once
/**
 * TRUE O(n + r log r) Polygon Triangulation - Version 2
 * 
 * Key fix: Track pending merge helpers and resolve them during chain advancement.
 * 
 * When a regular vertex on the left boundary is "passed" during lazy advancement,
 * if the chain's helper is a MERGE vertex, we add a diagonal from that regular
 * vertex to the merge helper.
 */
...
private:
  ...
  std::vector<int> start_chain_;
  std::vector<int> end_chain_;
  ...
  // Check if vertex v is on the left boundary (interior to its right)
  bool is_left_boundary_vertex(int v) const {
    int p = prv(v), nx = nxt(v);
    // On left boundary if going from v to nxt(v), interior is to our right
    // This happens when prv(v) is above v and nxt(v) is below v
    return below(nx, v) && !below(p, v);
  }
  
public:
  // Chain advancement that tracks regular vertices for diagonal insertion
  double chain_x_at_with_diags(int cid, double y) {
    Chain& ch = chains_[cid];
    int len = static_cast<int>(ch.verts.size());
    
    // Advance and check for regular vertex diagonal opportunities
    while (ch.pos + 1 < len && pts_[ch.verts[ch.pos + 1]].y > y + kEps) {
      int passed_v = ch.verts[ch.pos + 1];
      
      // If we're passing a REGULAR vertex on the left boundary,
      // and the helper is a MERGE vertex, add diagonal
      if (types_[passed_v] == REGULAR && is_left_boundary_vertex(passed_v)) {
        if (ch.helper >= 0 && types_[ch.helper] == MERGE && ch.helper != passed_v) {
          diagonals.push_back({passed_v, ch.helper});
          ch.helper = passed_v;  // Update helper after diagonal
        }
      }
  ...
  int slab_vertex(int cid) const {
    return chains_[cid].verts[chains_[cid].pos];
  }
...
inline void Triangulator::triangulate(const std::vector<Point>& input) {
  triangles.clear();
  diagonals.clear();
  reflex_count = 0;
  ...
  if (area < 0) std::reverse(pts_.begin(), pts_.end());
...
inline void Triangulator::classify() {
  types_.resize(n_);
  reflex_count = 0;
...
inline void Triangulator::build_chains() {
  chains_.clear();
...
inline void Triangulator::decompose() {
  ...
  for (const Event& ev : events) {
    ...
    // Before processing, advance all active chains to current y
    // This triggers diagonal insertion for passed regular vertices
    for (int cid : active) {
      chain_x_at_with_diags(cid, sweep_y_);
    }
...
inline void Triangulator::extract_and_triangulate() {
  // Remove duplicate diagonals
  for (auto& d : diagonals) {
    if (d.first > d.second) std::swap(d.first, d.second);
  }
  std::sort(diagonals.begin(), diagonals.end());
  diagonals.erase(std::unique(diagonals.begin(), diagonals.end()), diagonals.end());
  
  // Linked-list face extraction
  int maxv = n_ + 2 * static_cast<int>(diagonals.size());
  
  struct LLV { double x, y; int next, prev, orig; };  // Added orig field
  std::vector<LLV> ll(maxv);
  int num_ll = n_;
  
  for (int i = 0; i < n_; ++i) {
    ll[i] = {pts_[i].x, pts_[i].y, nxt(i), prv(i), i};  // orig = i
  }
  ...
  for (auto& d : diagonals) {
    ...
    ll[n1] = ll[i1];  // Copies including orig
    ll[n2] = ll[i2];
  ...
  for (int s = 0; s < num_ll; ++s) {
    ...
    if (face.size() >= 3) {
      for (size_t i = 1; i + 1 < face.size(); ++i) {
        int v0 = ll[face[0]].orig;
        int v1 = ll[face[i]].orig;
        int v2 = ll[face[i + 1]].orig;
