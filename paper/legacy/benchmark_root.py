...
def write_tex_tables(summary: Dict[Tuple[str, int], dict], sizes: List[int], polygon_types: List[str]) -> None:
    ...
    for n in sizes:
        ...
        for _, m, s in cols:
            ...
            if best is not None and m is not None and abs(m - best) < 1e-12:
                cell = r"\textbf{" + cell + "}"
            vals.append(cell)
        lines.append(f"{n:,} & {k_avg} & " + " & ".join(vals) + r" \\")
    ...
    for ptype in polygon_types:
        ...
        for n in sizes:
            ...
            first = False
            lines.append(f"{type_col} & {n:,} & {k_avg} & " + " & ".join(vals) + r" \\")
        lines.append(r"\midrule")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for ptype, gen in polygon_types.items():
            for n in sizes:
                ...
                k_avg = statistics.mean(k_vals) if k_vals else 0.0
                record = {"type": ptype, "n": n, "k_avg": k_avg, "k_ratio": (k_avg / n if n else 0.0)}
    ...
    for ptype in ["convex", "dent", "random", "star"]:
        ...
        lines.append("n\t k(avg)\t " + "\t ".join(f"{a}(ms)" for a in alg_order) + "\n")
        for n in sizes:
            rec = summary[(ptype, n)]
            cells = [pm3(rec.get(f"{a}_mean"), rec.get(f"{a}_std")) for a in alg_order]
            lines.append(f"{n}\t {int(rec['k_avg'])}\t " + "\t ".join(cells) + "\n")
        lines.append("\n")
