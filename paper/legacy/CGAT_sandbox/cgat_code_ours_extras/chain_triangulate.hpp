 ...
 * - Phase 1 (vertex classification + chain construction): O(n)
 * - Phase 2 (monotone decomposition via chain sweep): O(k log k)
 *     - 2k events (k local maxima + k local minima)
 *     - BST (treap) of size O(k) chains
 *     - Lazy chain advancement: O(n) total (amortized)
 * - Phase 3 (face extraction + triangulation): O(n)
 *     - Uses PolyPartition-style linked-list split: O(1) per diagonal
 *     - Monotone face triangulation: O(face size)
 *
 * Total: O(n + k log k)
 *
...
public:
  ...
  void triangulate(std::vector<Point>& pts) {
    ...
    // Monotone decomposition (O(k log k) sweep + O(n) lazy advancement)
    decompose_monotone(pts);

    // Extract and triangulate faces (O(n) total)
    triangulate_faces(pts);
  }

private:
  enum class VType : uint8_t { Start, End, Split, Merge, Regular };

  // Linked-list vertex (PolyPartition style)
  struct Vertex {
    double x, y;
    int orig_idx;  // original point index
    int prev, next;
  };
  ...
  void init_linked_list(const std::vector<Point>& pts) {
    const int n = static_cast<int>(pts.size());
    // Reserve extra space for diagonal vertex duplications
    verts_.resize(3 * n);
    num_verts_ = n;
  ...
  // Add diagonal in O(1) using linked-list split (PolyPartition style)
  // Creates two new vertex copies and rewires to split the polygon into two loops.
  void add_diagonal(int index1, int index2) {
    if (index1 == index2) return;
    if (index1 >= num_verts_ || index2 >= num_verts_) return;

    // Create new vertex copies
    ...
    // PolyPartition rewiring:
    // newindex2.next = index2.next
    // newindex1.next = index1.next
    verts_[newindex2].next = verts_[index2].next;
    verts_[newindex1].next = verts_[index1].next;

    // Fix back pointers for the vertices that were next
    // index2.next.prev = newindex2
    // index1.next.prev = newindex1
    verts_[verts_[index2].next].prev = newindex2;
    verts_[verts_[index1].next].prev = newindex1;

    // Create diagonal edges
    // index1.next = newindex2, newindex2.prev = index1
    verts_[index1].next = newindex2;
    verts_[newindex2].prev = index1;

    // index2.next = newindex1, newindex1.prev = index2
    verts_[index2].next = newindex1;
    verts_[newindex1].prev = index2;

    // Result: two loops
    // Loop 1: index1 -> newindex2 -> (old index2.next) -> ... -> index1
    // Loop 2: index2 -> newindex1 -> (old index1.next) -> ... -> index2
  }
  ...
  // Phase 2: O(k log k) decomposition
  ...
  // Phase 3: O(n) face extraction + triangulation
  void triangulate_faces(const std::vector<Point>& pts) {
    ...
    for (int start = 0; start < num_verts_; ++start) {
      ...
      do {
        ...
        face.push_back(cur);
        cur = verts_[cur].next;
        if (++safety > num_verts_ + 10) break;
      } while (cur != start);

      if (face.size() < 3) continue;

      // Compute area to determine interior/exterior
      double area = 0.0;
  ...
  // Helper struct for monotone triangulation
  struct FaceVertex {
    double x, y;
    int orig_idx;
  };
  ...
  void triangulate_monotone_face(const std::vector<int>& face, const std::vector<Point>& pts) {
    ...
    // Build face vertex array with coordinates
    std::vector<FaceVertex> fv(m);
    for (int i = 0; i < m; ++i) {
      fv[i].x = verts_[face[i]].x;
      fv[i].y = verts_[face[i]].y;
      fv[i].orig_idx = verts_[face[i]].orig_idx;
    }

    if (m == 3) {
      triangles.push_back({fv[0].orig_idx, fv[1].orig_idx, fv[2].orig_idx});
      return;
    }
    ...
    for (int i = 1; i < m; ++i) {
      if (fv[i].y > fv[top_idx].y || (fv[i].y == fv[top_idx].y && fv[i].x < fv[top_idx].x))
        top_idx = i;
      if (fv[i].y < fv[bot_idx].y || (fv[i].y == fv[bot_idx].y && fv[i].x > fv[bot_idx].x))
        bot_idx = i;
    }

    // Build left chain (CCW from top to bottom)
    ...
    // Build right chain (CCW from bottom to top, then reverse)
    ...
    right_chain.push_back(top_idx);
    std::reverse(right_chain.begin(), right_chain.end());

    // Merge into sorted list
    std::vector<std::pair<int, bool>> sorted;  // (face index, is_left)
    size_t li = 0, ri = 0;
    ...
    while (li < left_chain.size() || ri < right_chain.size()) {
      ...
      } else if (above_fv(left_chain[li], right_chain[ri])) {
        sorted.push_back({left_chain[li++], true});
      } else {
        sorted.push_back({right_chain[ri++], false});
      }
    ...
    for (size_t j = 2; j < sorted.size(); ++j) {
      ...
      if (side_j != stack.back().second) {
        while (stack.size() > 1) {
          int v1 = stack.back().first;
          stack.pop_back();
          int v2 = stack.back().first;
          triangles.push_back({fv[vj].orig_idx, fv[v1].orig_idx, fv[v2].orig_idx});
        }
        stack.clear();
